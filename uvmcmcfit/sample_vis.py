...
def uvmodel(model, modelheader, u, v, pcd):
    ...
    raref2 = (pcd[0] - pcm[0]) * np.cos(pcd[1] * pi / 180.0) * pi / 180.0
    decref2 = (pcd[1] - pcm[1]) * pi / 180.0
    ...
    if igrid == 1:
        ...
        #        time_modgrid = time_modgrid + time.time()-start
        #        start = time.time()
        mvis_opt = grid.ModShift(
            ud, vd, raref1, decref1, raref2, decref2, 1, 1, mvis_opt
        )
        # for i in np.arange(nvis):
        #    if (np.abs(uu[i]) > umax) or (np.abs(vv[i]) > vmax):
        #        mvis_opt[i] = 0.
    ...
    if igrid == 2:
        ...
        mvis_opt = np.zeros(nvis) + 1.0j * np.zeros(nvis)
        mv1d = -1 * mu1d
        ...
        for i in np.arange(nvis):
            ...
            mvis_opt[i] = grid.ModShift(
                ud[i], vd[i], raref1, decref1, raref2, decref2, 1, 1, mvis_opt[i]
            )
            time_modgrid = time_modgrid + time.time() - start
        # print(time_modgrid)
