...
def makeScript(parameters):
    ...
    f.write(
        "invert vis=$VIS map=$OUT.map beam=$OUT.beam imsize=$IMSIZE cell=$CELL fwhm=$FWHM sup=$SUP options=systemp,mfs robust=$ROBUST \n"
    )
    f.write("rm -rf $OUT.cc $OUT.cc1 $OUT.cc2 $OUT.cm $OUT.cm1 $OUT.cm2 \n")
    f.write(
        "clean map=$OUT.map beam=$OUT.beam out=$OUT.cc gain=$GAIN niters=$NITERS region=$REGION cutoff=$CUTOFF \n"
    )
    f.write(
        "clean map=$OUT.map beam=$OUT.beam out=$OUT.cc2 gain=$GAIN niters=$NITERS region=$REGION2 cutoff=$CUTOFF2 \n"
    )
    f.write(
        "clean map=$OUT.map beam=$OUT.beam out=$OUT.cc1 gain=$GAIN niters=$NITERS region=$REGION2 cutoff=$CUTOFF2 model=$OUT.cc \n"
    )
